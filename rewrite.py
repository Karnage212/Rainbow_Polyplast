...
# 1. Replace favicons and meta tags with .png
html = re.sub(
    r'<link rel="icon" type="image/svg\+xml" href="\./assets/logo_assets/svg/logo-icon-transparent-bg\.svg">',
    '<link rel="icon" type="image/png" href="./assets/ICON RP.png">',
    html
)
html = re.sub(
    r'<meta property="og:image" content="[^"]+">',
    '<meta property="og:image" content="https://www.rainbowpolypack.com/assets/ICON RP.png">',
    html
)
html = re.sub(
    r'<meta property="twitter:image" content="[^"]+">',
    '<meta property="twitter:image" content="https://www.rainbowpolypack.com/assets/ICON RP.png">',
    html
)
html = html.replace(
    '"image": "https://www.rainbowpolypack.com/assets/logo_assets/svg/polypack-logo-transparent-bg.svg"',
    '"image": "https://www.rainbowpolypack.com/assets/ICON RP.png"'
)
html = html.replace(
    '"logo": "https://www.rainbowpolypack.com/assets/logo_assets/svg/polypack-logo-transparent-bg.svg"',
    '"logo": "https://www.rainbowpolypack.com/assets/ICON RP.png"'
)
html = html.replace(
    '"url": "https://www.rainbowpolypack.com/assets/logo_assets/svg/polypack-logo-transparent-bg.svg"',
    '"url": "https://www.rainbowpolypack.com/assets/ICON RP.png"'
)
...
navbar_code = """    function Navbar() {
      ...
      const links = [
        ...
        { path: '/sustainability', label: t("nav_sustainability") }
      ];
      ...
      return html`
        <header className=${`fixed top-0 left-0 right-0 z-[60] transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-md py-3' : 'bg-transparent py-6'}`}>
          <div className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between">
            <div
              onClick=${() => navigate('/')}
              className="cursor-pointer transition-transform duration-300 hover:scale-[1.02]"
            >
              <img src="./assets/ICON RP.png" width="60" height="60" className=${`w-auto object-contain transition-all duration-300 ${scrolled ? 'h-10' : 'h-14'}`} alt="Rainbow Polypack Logo" />
            </div>
        ...
        <${AnimatePresence}>
          ${isOpen && html`
            <${motion.div}
              ...
              exit=${{ opacity: 0, y: -10 }}
              className="fixed top-[70px] left-4 right-4 z-[50] md:hidden bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-xl"
            >
              <div className="p-4 space-y-1">
                ${links.map(link => {
                  ...
                  <div
                    key=${link.path}
                    onClick=${() => handleNav(link.path)}
                    className=${`cursor-pointer block px-4 py-3 rounded-xl text-sm font-bold transition-colors ${isActive ? 'bg-brand-green/10 text-brand-green' : 'text-slate-600 hover:bg-slate-50'}`}
                  >
                    ${link.label}
                  </div>
                `})}
...
home_code = """    function Home() {
      ...
      return html`
        <div className="bg-brand-white w-full font-sans text-slate-800">
          ...
          <div className="relative pt-32 pb-16 md:pt-48 md:pb-24 px-6 max-w-7xl mx-auto flex flex-col items-center text-center">
            <div className="inline-block px-4 py-1.5 rounded-full bg-brand-green/10 text-brand-green font-bold text-xs uppercase tracking-widest mb-6">ISO 9001:2015 Compliant Partner</div>
            <h1 className="text-4xl md:text-6xl lg:text-7xl font-black text-brand-black tracking-tight mb-6 leading-tight">
              Premium PET Preform <br className="hidden md:block" /> Manufacturing
            </h1>
            <p className="text-lg md:text-xl text-slate-600 max-w-3xl mb-10 font-medium leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
              <${Link} to="/brochure" className="px-8 py-4 bg-brand-green text-white rounded-xl font-bold text-lg hover:bg-brand-black transition-colors shadow-lg flex items-center justify-center">
                Request a Technical Quote
              <//>
              <a href="#products" className="px-8 py-4 bg-white text-brand-black border border-slate-200 rounded-xl font-bold text-lg hover:bg-slate-50 transition-colors shadow-sm flex items-center justify-center">
                View Catalog
              </a>
            </div>
          </div>

          <!-- Lazy-loaded Inline Video Block -->
          <div className="px-6 max-w-6xl mx-auto mb-24 relative z-10">
            <div className="relative w-full rounded-3xl overflow-hidden shadow-2xl bg-brand-black aspect-video border border-slate-200/50">
              <video 
                ref=${mobileVideoRef}
          ...
          <div className="bg-white py-24 border-y border-slate-100">
            <div className="max-w-7xl mx-auto px-6">
              <div className="text-center mb-16">
                <h2 className="text-3xl md:text-5xl font-black text-brand-black mb-6 tracking-tight">Engineering Excellence</h2>
                <p className="text-slate-600 max-w-2xl mx-auto text-lg">Our state-of-the-art facility guarantees commercial-scale production with precision tolerances, empowering your blowing lines.</p>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ${features.map(feature => html`
                  <div key=${feature.id} className="p-8 rounded-3xl bg-brand-white border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <div className="text-brand-green mb-6 bg-white shadow-sm w-16 h-16 rounded-2xl flex items-center justify-center border border-slate-50">
                      ${feature.icon}
                    </div>
...
# Replace Footer Logo & styling (polypack-logo-transparent-bg.svg -> FONTs.png)
html = re.sub(
    r'<img src="\./assets/logo_assets/svg/polypack-logo-transparent-bg\.svg"[^>]+>',
    '<img src="./assets/FONTs.png" width="600" height="200" className="w-64 sm:w-80 h-auto opacity-90 object-contain mx-auto" alt="Rainbow Polypack Logo" />',
    html
)
